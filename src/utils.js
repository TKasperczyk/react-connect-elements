...
export const drawPath = (svg, path, startX, startY, endX, endY, svgHeight, color) => {
  ...
  path.setAttribute(
    ...
    `M${startX} ${startY} 
     H ${startX + deltaX} 
     Q ${startX + 2 * deltaX} ${startY} ${startX + 2 * deltaX} ${(endY + startY) / 2}
...
export const connectElements = (container, svg, path, startElem, endElem, color) => {
  // if first element is further than the second, swap!
  if (
    startElem.getBoundingClientRect().left > endElem.getBoundingClientRect().left
  ) {
